...
void KnowledgeExplorerPage::setupUI() {
    ...
    m_exportCodeBtn = new QPushButton("💾 另存为 .cpp", codeCard);
    m_exportCodeBtn->setCursor(Qt::PointingHandCursor);
    connect(m_exportCodeBtn, &QPushButton::clicked, this, &KnowledgeExplorerPage::exportCodeToFile);

    codeHeader->addWidget(codeTitle);
    codeHeader->addStretch();
    codeHeader->addWidget(m_copyCodeBtn);
    codeHeader->addWidget(m_exportCodeBtn);
    ...
    auto *exportFullBtn = new QPushButton("💾 另存为 .cpp", codeFullCard);
    exportFullBtn->setCursor(Qt::PointingHandCursor);
    connect(exportFullBtn, &QPushButton::clicked, this, [this]() {
        QString code = m_fullCodeEdit ? m_fullCodeEdit->toPlainText() : "";
        if (code.trimmed().isEmpty()) return;
        QString defaultName = QString("%1_arch.cpp").arg(m_currentTopicId.isEmpty() ? "qt_module" : m_currentTopicId);
        QString filePath = QFileDialog::getSaveFileName(this, "导出 Qt 架构范式代码", defaultName, "C++ Source (*.cpp *.h);;All Files (*.*)");
        if (filePath.isEmpty()) return;
        QFile file(filePath);
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QTextStream out(&file);
            out << "// ========================================================\n";
            out << "// Auto-generated by VisionCraft Knowledge Lab\n";
            out << "// Topic: " << (m_topicTitleLabel ? m_topicTitleLabel->text() : "") << "\n";
            out << "// API: " << (m_apiSignatureLabel ? m_apiSignatureLabel->text() : "") << "\n";
            out << "// ========================================================\n\n";
            out << code;
            file.close();
        }
    ...
    codeFullHeader->addWidget(exportFullBtn);
...
void KnowledgeExplorerPage::exportCodeToFile() {
    QString code = m_codeEdit ? m_codeEdit->toPlainText() : "";
    if (code.trimmed().isEmpty()) return;

    QString defaultName = QString("%1_snippet.cpp").arg(m_currentTopicId.isEmpty() ? "vision_operator" : m_currentTopicId);
    QString filePath = QFileDialog::getSaveFileName(this, "导出 C++ 算子代码片段", defaultName, "C++ Source (*.cpp *.h);;All Files (*.*)");
    if (filePath.isEmpty()) return;
