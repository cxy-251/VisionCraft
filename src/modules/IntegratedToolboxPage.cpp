...
#include "node_pipeline/NodeGraphPipelinePage.h"
#include "batch_inspector/BatchInspectorPage.h"
...
IntegratedToolboxPage::IntegratedToolboxPage(QWidget *parent) : IToolPage(parent) {
    ...
    auto updateTabsStyle = [this](bool isDark) {
        if (isDark) {
            m_tabWidget->setStyleSheet(
                ...
                "QTabBar::tab { font-size: 13px; font-weight: 600; padding: 10px 18px; border-top-left-radius: 8px; border-top-right-radius: 8px; margin-right: 4px; }"
    ...
    m_nodeGraphPage = new NodeGraphPipelinePage(this);
    m_batchInspectorPage = new BatchInspectorPage(this);
    ...
    m_tabWidget->addTab(m_nodeGraphPage, "🧩 节点图视觉流水线");
    m_tabWidget->addTab(m_batchInspectorPage, "🏭 工业质检批处理中心");
    ...
    connect(m_tabWidget, &QTabWidget::currentChanged, this, [this](int idx) {
        ...
        if (idx == 1) m_nodeGraphPage->onActivated();
        else m_nodeGraphPage->onDeactivated();

        if (idx == 2) m_batchInspectorPage->onActivated();
        else if (idx == 3) m_matcherPage->onActivated();
        else if (idx == 4) m_processorPage->onActivated();
        else if (idx == 5) m_devPage->onActivated();
    });
...
void IntegratedToolboxPage::onActivated() {
    ...
    else if (cur == 1) m_nodeGraphPage->onActivated();
    else if (cur == 2) m_batchInspectorPage->onActivated();
    else if (cur == 3) m_matcherPage->onActivated();
    else if (cur == 4) m_processorPage->onActivated();
    else if (cur == 5) m_devPage->onActivated();
}

void IntegratedToolboxPage::onDeactivated() {
    m_livePipelinePage->onDeactivated();
    m_nodeGraphPage->onDeactivated();
